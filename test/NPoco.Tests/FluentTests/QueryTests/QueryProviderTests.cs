...
namespace NPoco.Tests.FluentTests.QueryTests
{
    ...
    public class QueryProviderTests : BaseDBFluentTest
    {
        ...
        [Test]
        public void QueryWithWhereIf()
        {
            var users = Database.Query<User>().WhereIf(true, x => x.UserId > 10).ToList();
            var inmemory = InMemoryUsers.Where(x => x.UserId > 10).ToList();
        ...
        [Test]
        public void QueryWithWhereIfAnd()
        {
            var users = Database.Query<User>().WhereIf(true, x => x.UserId > 10 && x.UserId < 12).ToList();
            var inmemory = InMemoryUsers.Where(x => x.UserId > 10 && x.UserId < 12).ToList();
        ...
        public void QueryWithProjectionAndEnclosedMethod1()
        {
            ...
            try
            {
                ...
                // NewArrayExpression).
                var users = Database.Query<User>()
                    .ProjectTo(x => new ProjectUser2
                    {
                        FormattedAge = string.Format("{0:n2} {1:n2} {2:n2} {3:n2} {4:n2} {5:n2} {6:n2}",
